<template>
	<div class="h-screen flex flex-col overflow-hidden" :class="isDark ? 'bg-slate-950' : 'bg-white'">
		<!-- Header -->
		<header class="flex items-center justify-between h-12 px-4 border-b shrink-0" 
			:class="isDark ? 'border-slate-800/80 bg-slate-950' : 'border-slate-200 bg-white'">
			<!-- Title -->
			<div class="flex items-center gap-2 min-w-0 overflow-hidden">
				<svg class="w-5 h-5 shrink-0" :class="isDark ? 'text-cyan-400' : 'text-cyan-600'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
				</svg>
				<span class="text-sm font-medium truncate" :class="isDark ? 'text-white' : 'text-slate-900'">
					Cartographer
				</span>
			</div>
			...
			<div class="flex items-center gap-1.5">
				...
				>
					<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
						<path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM7 10h6" />
					</svg>
				</button>
				<button
					@click="resetZoom"
		...
		<div class="flex-1 relative overflow-hidden">
			...
			<div v-if="loading" class="absolute inset-0 flex items-center justify-center">
				<div class="text-center">
					...
					<p class="text-sm" :class="isDark ? 'text-slate-400' : 'text-slate-500'">Loading network map...</p>
				</div>
			</div>
