			...
			<div class="p-6">
				...
				<template v-else>
					...
					<div class="mb-6 p-4 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
						...
						<div v-if="saving" class="mt-2 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
					...
					<!-- Info about server-side setting -->
					<div class="mb-6 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50">
						<div class="flex items-start gap-2">
							...
							<div class="text-sm text-blue-800 dark:text-blue-200">
								<strong>Server-side setting:</strong> The sensitive mode setting is stored on the server and applies to all embed viewers. It cannot be bypassed via URL parameters.
							</div>
						</div>
					</div>
...
async function toggleSensitiveMode() {
	saving.value = true;
	const newValue = !sensitiveMode.value;
	try {
		await axios.post('/api/embed-config', { sensitiveMode: newValue });
		sensitiveMode.value = newValue;
	} catch (err) {
