...
class TestContextEndpoints:
    ...
    def test_get_context(self, client):
        ...
        with patch('app.routers.assistant.metrics_context_service') as mock_service:
            mock_service.build_context_string = AsyncMock(return_value=(
                "Context",
                {"total_nodes": 5, "healthy_nodes": 4, "unhealthy_nodes": 1, "gateway_count": 1, "context_tokens_estimate": 100}
            ))
...
class TestContextRawEndpoint:
    ...
    def test_get_context_raw_success(self, client):
        ...
        with patch('app.routers.assistant.metrics_context_service') as mock_service:
            mock_service.fetch_network_snapshot = AsyncMock(return_value={
                "nodes": {
                    "node-1": {"name": "Test", "ip": "192.168.1.1", "role": "server", "notes": None, "isp_info": None}
                },
                "gateways": [
