...
class TestProviderEdgeCases:
    ...
    async def test_gemini_list_models_filters(self):
        ...
        with patch.object(provider, '_configure_genai') as mock_genai:
            mock_genai.return_value.list_models = MagicMock(return_value=[mock_chat_model, mock_embed_model])
...
class TestMetricsContextEdgeCases:
    ...
    async def test_format_gateway_test_ips_dict_status(self):
        ...
        gateway = {
            ...
            "test_ips": [
                {"ip": "8.8.8.8", "label": "Google", "status": {"value": "healthy"}}
            ],
    ...
    async def test_build_context_with_roles(self):
        ...
        snapshot = {
            ...
            "unknown_nodes": 0,
            "nodes": {
                "gw": {"id": "gw", "name": "Gateway", "ip": "192.168.1.1", "role": "gateway/router", "status": "healthy"},
                "fw": {"id": "fw", "name": "Firewall", "ip": "192.168.1.2", "role": "firewall", "status": "healthy"},
                "nas": {"id": "nas", "name": "NAS", "ip": "192.168.1.3", "role": "nas", "status": "healthy"},
                "client": {"id": "client", "name": "Laptop", "ip": "192.168.1.4", "role": "client", "status": "healthy"},
            },
            "connections": [],
            "gateways": []
        }
    ...
    async def test_build_context_with_unknown_role(self):
        ...
        snapshot = {
            ...
            "unknown_nodes": 0,
            "nodes": {
                "unknown": {"id": "unknown", "name": "Unknown Device", "ip": "192.168.1.1", "role": "unknown", "status": "healthy"},
            },
            "connections": [],
            "gateways": []
        }
    ...
    async def test_format_lan_ports_with_poe(self):
        ...
        lan_ports = {
            ...
            "cols": 4,
            "ports": [
                {"row": 1, "col": 1, "status": "active", "type": "rj45", "poe": "poe+", "connected_device_name": "Camera", "speed": "1G"},
            ]
        }
...
class TestRouterEdgeCases:
    ...
    def test_stream_error_handling(self):
        ...
        with patch('app.routers.assistant.get_provider') as mock_get:
            ...
            with patch('app.routers.assistant.metrics_context_service') as mock_service:
                mock_service.build_context_string = AsyncMock(return_value=("Context", {"test": True}))
                
                response = client.post("/api/assistant/chat/stream", json={
                    "message": "Hello",
                    "provider": "openai"
                })
