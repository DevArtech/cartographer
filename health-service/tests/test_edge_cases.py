...
"""
    ...
    async def test_ping_calculates_jitter(self, health_checker_instance, mock_subprocess_ping_success):
...
class TestHistoricalStatsEdgeCases:
    ...
    def test_old_history_excluded(self, health_checker_instance):
        ...
        uptime, avg_lat, passed, failed = health_checker_instance._calculate_historical_stats("192.168.1.1")
...
class TestCheckDeviceHealthEdgeCases:
    ...
    async def test_consecutive_failures_incremented(self, health_checker_instance, mock_ping_failure):
        ...
        with patch('app.services.health_checker.report_health_check', new_callable=AsyncMock):
            # First failure
            metrics1 = await health_checker_instance.check_device_health("192.168.1.1", include_dns=False)
            assert metrics1.consecutive_failures == 1
            
            # Second failure
            metrics2 = await health_checker_instance.check_device_health("192.168.1.1", include_dns=False)
            assert metrics2.consecutive_failures == 2
    
    async def test_consecutive_failures_reset_on_success(self, health_checker_instance, mock_ping_success, mock_ping_failure):
        ...
        with patch('app.services.health_checker.report_health_check', new_callable=AsyncMock):
            ...
            metrics = await health_checker_instance.check_device_health("192.168.1.1", include_dns=False)
...
class TestTestIPEdgeCases:
    ...
    def test_calculate_test_ip_stats_empty(self, health_checker_instance):
        ...
        uptime, avg_lat, passed, failed = health_checker_instance._calculate_test_ip_historical_stats(
            "192.168.1.1", "8.8.8.8"
        )
    ...
    async def test_check_test_ip_increments_failures(self, health_checker_instance, mock_ping_failure):
    ...
    async def test_check_gateway_test_ips_handles_exception(self, health_checker_instance, sample_gateway_test_ips):
    ...
    async def test_speed_test_stores_result_for_gateway(self, health_checker_instance, tmp_path):
        ...
        with patch('speedtest.Speedtest', mock_speedtest):
            with patch('app.services.health_checker.DATA_DIR', tmp_path):
                with patch('app.services.health_checker.SPEED_TEST_RESULTS_FILE', tmp_path / "speed.json"):
...
class TestMonitoringLoopEdgeCases:
    ...
    async def test_perform_monitoring_check_with_gateway_test_ips(self, health_checker_instance, sample_gateway_test_ips, mock_ping_success):
...
class TestCheckHistoryEntry:
    ...
    def test_get_test_ip_check_history_with_hours(self, health_checker_instance):
        ...
        history = health_checker_instance._get_test_ip_check_history("192.168.1.1", "8.8.8.8", hours=6)
        assert len(history) == 1
        
        # 24 hour window should get both
        history_24h = health_checker_instance._get_test_ip_check_history("192.168.1.1", "8.8.8.8", hours=24)
