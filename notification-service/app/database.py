...
async def _migrate_network_id_to_uuid(conn):
    ...
    if row and row[0] in ('integer', 'bigint'):
        ...
        await conn.execute(text("ALTER TABLE discord_user_links RENAME COLUMN context_id_new TO context_id"))
    ...
    if row and row[0] in ('integer', 'bigint'):
        ...
        await conn.execute(text("ALTER TABLE user_network_notification_prefs DROP COLUMN network_id"))
        await conn.execute(text("""
