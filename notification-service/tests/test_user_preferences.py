...
class TestUserPreferencesServiceUpdate:
    ...
    async def test_update_network_preferences(self):
        ...
        with patch.object(service, 'get_or_create_network_preferences', new_callable=AsyncMock) as mock_get:
    ...
    async def test_update_global_preferences(self):
        ...
        with patch.object(service, 'get_or_create_global_preferences', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = mock_prefs
            mock_db.commit = AsyncMock()
            mock_db.refresh = AsyncMock()
