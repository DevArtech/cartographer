...
class TestDataFetching:
    ...
    async def test_fetch_health_metrics_success(self, metrics_aggregator_instance, sample_health_metrics):
    ...
    async def test_fetch_gateway_test_ips_success(self, metrics_aggregator_instance, sample_gateway_test_ips):
    ...
    async def test_fetch_gateway_test_ips_fallback(self, metrics_aggregator_instance, sample_gateway_test_ips):
    ...
    async def test_fetch_speed_test_results_success(self, metrics_aggregator_instance, sample_speed_test_results):
...
class TestDataTransformation:
    ...
    def test_parse_device_role_valid(self, metrics_aggregator_instance):
        ...
        assert metrics_aggregator_instance._parse_device_role("gateway/router") == DeviceRole.GATEWAY_ROUTER
        ...
        assert metrics_aggregator_instance._parse_device_role("GROUP") == DeviceRole.GROUP  # Case insensitive
    ...
    def test_parse_health_status_valid(self, metrics_aggregator_instance):
        ...
        assert metrics_aggregator_instance._parse_health_status("unhealthy") == HealthStatus.UNHEALTHY
    ...
    def test_transform_check_history_invalid(self, metrics_aggregator_instance):
        ...
        history = [
            {"timestamp": "invalid", "success": True}  # Invalid timestamp
        ]
    ...
    def test_transform_lan_ports_invalid_values(self, metrics_aggregator_instance):
        ...
        lan_ports_data = {
            ...
            "cols": 1,
            "ports": [
                {"row": 1, "col": 1, "type": "invalid_type", "status": "invalid_status", "poe": "invalid_poe"}
            ]
        }
...
class TestSnapshotGeneration:
    ...
    async def test_generate_snapshot_success(
        ...
        with patch.object(metrics_aggregator_instance, '_fetch_network_layout', AsyncMock(return_value=sample_layout)):
            with patch.object(metrics_aggregator_instance, '_fetch_health_metrics', AsyncMock(return_value=sample_health_metrics)):
                with patch.object(metrics_aggregator_instance, '_fetch_gateway_test_ips', AsyncMock(return_value=sample_gateway_test_ips)):
                    with patch.object(metrics_aggregator_instance, '_fetch_speed_test_results', AsyncMock(return_value=sample_speed_test_results)):
    ...
    async def test_generate_snapshot_no_layout(self, metrics_aggregator_instance):
        ...
        with patch.object(metrics_aggregator_instance, '_fetch_network_layout', AsyncMock(return_value=None)):
            with patch.object(metrics_aggregator_instance, '_fetch_health_metrics', AsyncMock(return_value={})):
                with patch.object(metrics_aggregator_instance, '_fetch_gateway_test_ips', AsyncMock(return_value={})):
                    with patch.object(metrics_aggregator_instance, '_fetch_speed_test_results', AsyncMock(return_value={})):
                        snapshot = await metrics_aggregator_instance.generate_snapshot()
    ...
    async def test_generate_snapshot_empty_root(self, metrics_aggregator_instance):
        ...
        with patch.object(metrics_aggregator_instance, '_fetch_network_layout', AsyncMock(return_value={})):
            with patch.object(metrics_aggregator_instance, '_fetch_health_metrics', AsyncMock(return_value={})):
                with patch.object(metrics_aggregator_instance, '_fetch_gateway_test_ips', AsyncMock(return_value={})):
                    with patch.object(metrics_aggregator_instance, '_fetch_speed_test_results', AsyncMock(return_value={})):
                        snapshot = await metrics_aggregator_instance.generate_snapshot()
...
class TestPublishing:
    ...
    async def test_publish_snapshot_success(self, metrics_aggregator_instance, sample_snapshot):
        ...
        with patch.object(metrics_aggregator_instance, 'generate_snapshot', AsyncMock(return_value=sample_snapshot)):
    ...
    async def test_publish_snapshot_no_snapshot(self, metrics_aggregator_instance):
        ...
        with patch.object(metrics_aggregator_instance, 'generate_snapshot', AsyncMock(return_value=None)):
            result = await metrics_aggregator_instance.publish_snapshot()
...
class TestConfiguration:
    ...
    def test_get_last_snapshot_network_id_not_found(self, metrics_aggregator_instance, sample_snapshot):
    ...
    def test_last_snapshot_backwards_compat_first_value(self, metrics_aggregator_instance, sample_snapshot):
...
class TestMultiTenantLayout:
    ...
    async def test_fetch_network_layout_with_network_id_success(self, metrics_aggregator_instance, sample_layout):
...
class TestGenerateAllSnapshots:
    ...
    async def test_generate_all_snapshots_success(self, metrics_aggregator_instance, sample_snapshot):
        """Should generate snapshots for all networks"""
        with patch.object(metrics_aggregator_instance, '_fetch_all_network_ids', 
                         AsyncMock(return_value=["network-1", "network-2"])):
            with patch.object(metrics_aggregator_instance, 'generate_snapshot', 
    ...
    async def test_generate_all_snapshots_no_networks(self, metrics_aggregator_instance, sample_snapshot):
    ...
    async def test_generate_all_snapshots_partial_failure(self, metrics_aggregator_instance, sample_snapshot):
...
class TestPublishAllSnapshots:
    ...
    async def test_publish_all_snapshots_success(self, metrics_aggregator_instance, sample_snapshot):
        """Should publish snapshots for all networks"""
        snapshots = {"network-1": sample_snapshot, "network-2": sample_snapshot}
        
        with patch.object(metrics_aggregator_instance, 'generate_all_snapshots', 
    ...
    async def test_publish_all_snapshots_partial_failure(self, metrics_aggregator_instance, sample_snapshot):
        ...
        with patch.object(metrics_aggregator_instance, 'generate_all_snapshots', 
                         AsyncMock(return_value=snapshots)):
            with patch('app.services.metrics_aggregator.redis_publisher') as mock_publisher:
                mock_publisher.publish_topology_snapshot = AsyncMock(side_effect=publish_with_failure)
                mock_publisher.store_last_snapshot = AsyncMock(return_value=True)
    ...
    async def test_publish_all_snapshots_exception(self, metrics_aggregator_instance, sample_snapshot):
